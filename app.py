...
st.session_state.setdefault("langOptions", ['English', '中文'])

def update_lang_options():
    # move the selected option to the front of the list if it is not already
    if st.session_state.selected_lang_option != st.session_state.langOptions[0]:
        st.session_state.langOptions.remove(st.session_state.selected_lang_option)
        st.session_state.langOptions.insert(0, st.session_state.selected_lang_option)
...
class ResultStorage:
    ...
    def create_directories(self):
        ...
        os.makedirs(
            os.path.join(self.full_path, "scalers"), exist_ok=True
        )  # 保存归一化器
    ...
    def save_feature_importance(self, ticker, df):
        """保存特征重要性结果"""
        path = os.path.join(
            self.full_path, "feature_importance", f"{ticker}_features.csv"
        )
        df.to_csv(path, index=False)
...
def calculate_technical_indicators(data, start_date=None, end_date=None):
    ...
    true_range = pd.concat(
        [high_low_range, high_close_prev, low_close_prev], axis=1
    ).max(axis=1)
    data["ATR"] = true_range.rolling(window=14, min_periods=1).mean()
    data[["Close_yes", "Open_yes", "High_yes", "Low_yes"]] = data[
        ["Close", "Open", "High", "Low"]
    ].shift(
        2
    )  # 前一天价格数据
...
# 导入AKShare数据
def get_akshare_enhanced_data(ticker, start_date, end_date, market_source="askshare"):
    ...
    try:
        ...
        def convert_to_standard_format(df, ticker, start_date, end_date):
            ...
            for src, dest in column_mappings.items():
                ...
                if src_lower in column_lower_mapping:
                    df.rename(
                        columns={column_lower_mapping[src_lower]: dest}, inplace=True
                    )
        ...
        def get_akshare_raw_data(
            ticker, start_date, end_date, market_source="askshare"
        ):
            """从AKShare获取原始股票数据"""
            try:
                if ticker.endswith((".SH", ".SZ")) or market_source == "askshare":
        ...
        # 获取数据
        data = get_akshare_raw_data(
            ticker, extended_start_date, end_date, market_source
        )
...
def calculate_technical_indicators_enhanced(data):
    ...
    for window in [5, 10, 20, 50, 200]:
        try:
            ...
            data[f"MA{window}"] = (
                data["Close"].shift(1).rolling(window=window, min_periods=1).mean()
            )
        except Exception as e:
...
def get_stock_data(
    ...
    try:
        if data_source == "yahoo":
            ...
            data = get_akshare_enhanced_data(
                ticker, start_date, end_date, market_source
            )
        else:
    ...
    if tomorrow_pred:
        ...
        ax.set_xlim(
            (
                ...
                else date - pd.Timedelta(days=10)
            ),
...
def train_and_predict_lstm(
    ...
    for epoch in range(num_epochs):
        ...
        if lang == "zh":
            progress_text.text(
                f"轮次 {epoch+1}/{num_epochs} - 训练损失: {avg_train_loss:.6f}, 验证损失: {avg_val_loss:.6f}"
            )
        else:
            progress_text.text(
        ...
        loss_data = pd.DataFrame(
            {chart_title[0]: [avg_train_loss], chart_title[1]: [avg_val_loss]},
            index=[epoch],
        )
    ...
    with torch.no_grad():
        ...
        y_pred_tomorrow = scaler_y.inverse_transform(
            y_pred_tomorrow.cpu().numpy().reshape(-1, 1)
        )
    ...
    # 设置图表语言
    ax.plot(
        test_indices, actual_earnings, label="Actual Cumulative Return", color="blue"
    )
    ax.plot(
        test_indices,
        predicted_earnings,
    ...
    def analyze_feature_importance():
        ...
        base_value = scaler_y.inverse_transform(
            base_output.cpu().detach().numpy().reshape(-1, 1)
        )[0][0]
        ...
        for i in range(feature_count):
            ...
            modified_input = torch.tensor(
                modified_data.reshape(1, n_steps, feature_count), dtype=torch.float32
            ).to(device)
            modified_output = model(modified_input)
            modified_value = scaler_y.inverse_transform(
                modified_output.cpu().detach().numpy().reshape(-1, 1)
            )[0][0]
...
def run_prediction(tickers, num_epochs=50, lang="en", storage=None):
    ...
    for i, ticker_name in enumerate(tickers):
        try:
            ...
            metrics, model, scaler_X, scaler_y = (
                train_and_predict_lstm(  # 修改：接收返回的模型和归一化器
                    ticker_name,
                    stock_data,
                    *stock_features,
    ...
    if prediction_metrics:
        if lang == "zh":
            summary = {
                "平均准确率": np.mean(
                    [m["accuracy"] for m in prediction_metrics.values()]
                ),
                "最佳股票": max(
                    prediction_metrics.items(), key=lambda x: x[1]["accuracy"]
                )[0],
                "最差股票": min(
                    prediction_metrics.items(), key=lambda x: x[1]["accuracy"]
                )[0],
    ...
    if summary:
        ...
        st.text(
            "没有可用的预测结果" if lang == "zh" else "No prediction results available"
        )
    ...
    def train(self, epoch=100, print_every=1, lang="en"):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % print_every == 0:
                if lang == "zh":
                    st.text(
                        f"迭代 {i + 1}. 奖励: {self.reward_function(self.weights):.6f}"
                    )
                else:
                    st.text(
                        f"Iteration {i + 1}. Reward: {self.reward_function(self.weights):.6f}"
                    )
        # 显示总训练时间
        if lang == "zh":
...
class Agent:
    ...
    def __init__(
        self, model, window_size, trend, skip, initial_money, ticker, lang="en"
    ):
        """
        初始化交易代理
    ...
    def buy(self, storage=None):
        ...
        for t in range(0, len(self.trend) - 1, self.skip):
            ...
            if t > 0:
                # 计算前一日总价值
                prev_value = starting_money + sum(
                    [self.trend[t] - p for p in inventory]
                )
                # 计算当前总价值
                current_value = initial_money + sum(
                    [self.trend[t] - p for p in inventory]
                )
                # 计算日收益率
                ...
                report_df = pd.DataFrame(
                    {
                        ...
                        "预期短缺 (95%)": [
                            performance_report["Expected Shortfall (95%)"]
                        ],
                    }
                )
            else:
        ...
        if self.lang == "zh":
            ...
            ax.plot(
                states_buy,
                [self.trend[i] for i in states_buy],
                "^",
                markersize=10,
                color="green",
                label="Buy",
            ...
            ax.plot(
                states_sell,
                [self.trend[i] for i in states_sell],
                "v",
                markersize=10,
                color="red",
                label="Sell",
            ...
            ax.set_title(
                f"{self.ticker} Trading Strategy - Total Gain: { market_ccy()}{total_gains:.2f} ({invest:.2f}%)"
            )
            ax.set_xlabel("Days")
            ax.set_ylabel("Price")
        else:
...
# ====================== 单只股票策略处理 ======================
def process_stock(
    ticker, window_size=30, initial_money=10000, iterations=100, lang="en", storage=None
):
    try:
        # 根据语言设置子标题
        if lang == "zh":
        ...
        # 根据语言设置成功信息
        if lang == "zh":
            st.success(f"{ticker} 交易完成! 总收益: {market_ccy()} {total_gains:.2f} ({invest:.2f}%)")
        else:
            st.success(
                f"{ticker} trading completed! Total Gain: {market_ccy()} {total_gains:.2f} ({invest:.2f}%)"
            )
...
def run_trading(tickers, initial_money=10000, iterations=100, lang="en", storage=None):
    ...
    for i, ticker in enumerate(tickers):
        ...
        if lang == "zh":
            ...
            status_text.text(
                f"Executing {ticker} trading strategy... ({i+1}/{len(tickers)})"
            )
    ...
    if trading_results and lang == "zh":
        ...
        st.text(
            f"表现最佳: {trading_df.loc[trading_df['投资回报率(%)'].idxmax()]['股票代码']} "
            f"({trading_df['投资回报率(%)'].max():.2f}%)"
        )
        st.text(
            f"表现最差: {trading_df.loc[trading_df['投资回报率(%)'].idxmin()]['股票代码']} "
        ...
        st.text(
            f"Best performer: {trading_df.loc[trading_df['ROI (%)'].idxmax()]['Ticker']} "
            f"({trading_df['ROI (%)'].max():.2f}%)"
        )
        st.text(
            f"Worst performer: {trading_df.loc[trading_df['ROI (%)'].idxmin()]['Ticker']} "
    ...
    try:
        ...
        if "model_cache" in st.session_state and ticker in st.session_state.model_cache:
            ...
            if lang == "zh":
                ...
                st.warning(
                    f"No trained model found for {ticker}, using simple moving average"
                )
...
TEXTS = {
    "en": {
        ...
        "market_source_ask": "A-share",
        "market_source_hk": "Hong Kong stocks",
        "market_source_us": "US stocks",
    },
    "zh": {
        "title": "📈 股票预测与交易策略系统",
...
def market_ccy():
    _source = st.session_state.marketSource
    if _source == 'askshare':
        return '¥'
    return '$'
...
def create_sidebar():
    with st.sidebar:
        ...
        selected_lang = st.selectbox(
            tr("language"), 
            options=st.session_state.langOptions,
            key="selected_lang_option",
            on_change=update_lang_options,
        )
        st.header(tr1("sidebar_header", lang_options[selected_lang]))
        
        st.session_state.lang = lang_options[selected_lang]
        # print("{} {} {} {}".format(default_lang, default_index, selected_lang, st.session_state.lang))
        ...
        if selected_data_source == "AKShare":
            # 股票市场选择
            st.subheader(tr("market_source"))
            # 根据市场可用性过滤选项
            market_source_options = {
                tr("market_source_ask"): "askshare",
                tr("market_source_hk"): "hkshare",
                tr("market_source_us"): "usshare",
            }
            selected_market_source = st.selectbox(
                tr("market_source"),
                options=list(market_source_options.keys()),
                key="market_source_select",
            )
            market_source = market_source_options[selected_market_source]
        else:
            market_source = "usshare"
        st.session_state.marketSource = market_source

        # 随机性控制选项
        st.subheader(tr("randomness_control"))
        enable_randomness = st.checkbox(
            tr("enable_randomness"), value=False, help=tr("randomness_explanation")
        )

        if not enable_randomness:
            seed_value = st.number_input(
                tr("random_seed"), value=42, help="固定此值可确保结果可复现"
            )
            set_seed(seed_value)
        else:
            # 使用时间作为种子
        ...
        tickers_input = st.text_input(
            tr("ticker_label"), tr("ticker_default"), key="tickers"
        )
        start_date = st.date_input(
            tr("start_date"), default_start_date, key="start_date"
        )
        end_date = st.date_input(tr("end_date"), default_end_date, key="end_date")
        initial_money = st.number_input(
            tr("initial_money"), 10000, 1000000, 10000, key="initial_money"
        )
        num_epochs = st.slider(tr("num_epochs"), 10, 200, 50, key="num_epochs")
        iterations = st.slider(tr("iterations"), 10, 200, 50, key="iterations")

        # 高级选项
        with st.expander(tr("advanced_options")):
            incremental_training = st.checkbox(
                tr("enable_incremental_training"), value=True
            )
            multi_timeframe = st.checkbox(tr("enable_multi_timeframe"), value=False)
            backtest_timeframe = st.selectbox(
                tr("backtest_timeframe"), [tr("daily"), tr("weekly"), tr("monthly")]
            )
...
def main():
    ...
    # 明日预测按钮
    if st.button(
        tr("run_prediction_button"), use_container_width=True, key="run_prediction_only"
    ):
    ...
    with st.expander(
        "系统性能监控"
        if st.session_state.lang == "zh"
        else "System Performance Monitor"
    ):
        if st.button(
            "检查系统资源"
            if st.session_state.lang == "zh"
            else "Check System Resources"
        ):
            ...
            if torch.cuda.is_available():
                if st.session_state.lang == "zh":
                    ...
                    st.info(
                        f"GPU Memory: {torch.cuda.mem_get_info()[1]/1024**3:.2f} GB"
                    )
            else:
                if st.session_state.lang == "zh":
    ...
    st.sidebar.markdown("---")
    st.sidebar.info(
        f"Current language: {'English' if st.session_state.lang == 'en' else '中文'}"
    )
...
def run_full_analysis(
    ...
    with st.spinner(
        "优化资源分配中..."
        if st.session_state.lang == "zh"
        else "Optimizing resource allocation..."
    ):
        optimizer = PerformanceOptimizer()
    ...
    with st.spinner(tr("training_model")):
        prediction_metrics = run_prediction(
            optimized_tickers,
            num_epochs=num_epochs,
    ...
    zip_path = shutil.make_archive(
        st.session_state.storage.full_path, "zip", st.session_state.storage.full_path
    )
    with open(zip_path, "rb") as f:
        st.download_button(
            tr("download_results"), f, file_name=f"results_{int(time.time())}.zip"
        )
    ...
    for ticker in tickers:
        ...
        if prediction:
            ...
            if lang == "zh":
                ...
                cols[1].text(market_ccy() + f"{prediction['当前价格']:.2f}")
                cols[2].text(market_ccy() + f"{prediction['明日预测价格']:.2f}")
            else:
                cols[0].text(prediction["Ticker"])
                cols[1].text(market_ccy() + f"{prediction['Current Price']:.2f}")
                cols[2].text(market_ccy() + f"{prediction['Predicted Price']:.2f}")
